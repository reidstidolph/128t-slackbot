...
function keepConfigQuestion(){
	...
	if (JSON.stringify(config) != JSON.stringify(configFromDisk)) {
		keepConfigQuestionParams = {
			text : "Make further changes? (y/n)",
			invalidText : 'Please enter "y" for "yes", or "n" for "no".',
			validation : /^(y|n)$/
		};
	} else {
		keepConfigQuestionParams = {
			text : "Modify config? (y/n)",
			invalidText : 'Please enter "y" for "yes", or "n" for "no".',
			validation : /^(y|n)$/
		};
	}
	...
	prompt.question(keepConfigQuestionParams, (answer)=> {
		if (answer === "y") {
			...
			process.stdout.write("\nOk, backing up this config. You can re-run setup to make changes at any time.\n\n");
			fs.writeFile(configFile, JSON.stringify(config, null, 3), { mode: 0o600 }, (error)=>{
				...
				if (JSON.stringify(config) != JSON.stringify(configFromDisk)) {
					process.stdout.write("Restart 128T-Slackbot for changes to become active.\n\n");
				}
				process.stdout.write("You can test your config with './128t-slackbot slacktest' and './128t-slackbot routertest'.\n...Goodbye!\n\n");
				prompt.close();
			});
		}
	});
	...
	prompt.question(modifyConfigQuestionParams, (answer)=> {
		if (answer === "1" || answer === 1) {
			...
			fs.writeFile(configFile, JSON.stringify(config, null, 3), { mode: 0o600 }, (error)=>{
				if (error) {
					console.log(error);
					process.stdout.write("Error. Unable to backup config to disk.\n\n");
					prompt.close();
					process.exit(1);
				}
				if (JSON.stringify(config) != JSON.stringify(configFromDisk)) {
	...
	fs.readFile(configFile, (error, configContents)=>{
		...
		if (error) {
			...
			try {
				...
				configFromDisk = JSON.parse(configContents);
			}
			catch(error) {
