...
function sendRequest(method, t128resource, onRequestComplete) {
	logger.log("info", `Preparing to request data: ${method} ${t128resource}`)
	...
	function reSendRequest() {
		...
		logger.log("notice", `retrying request for data: ${method} ${t128resource}`);
		rest.request(method, restResource, requestArgs, (error, response, data)=> {
			...
			if (error) {
				...
				// resend success!
				// make callback
				logger.log("info", `Retry request for data got: ${response.statusCode}: ${response.statusMessage}`);
	...
	rest.request(method, restResource, requestArgs, (error, response, data)=> {
		...
		if (error) {
			...
			logger.log("notice", `Data request got ${response.statusCode}: ${response.statusMessage}. Re-initializing.`);
			t128Init((error, token, config)=> {
				if (error) {
					// could not re-initialize.
					// run callback and pass back error.
					logger.log("warning", "Unable to initialize:", error);
			...
			// request success!
			// make callback
			logger.log("info", `Request for data got: ${response.statusCode}: ${response.statusMessage}`);
