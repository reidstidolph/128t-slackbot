...
function getAttachmentHostCPU(hostCore){
	console.log(hostCore);
	var hostCPUString = "";
	var cpuSumUtilization = 0;
	var averageCPUUtilization = 0;

	hostCore.forEach((core)=> {
		process.stdout.write(`${cpuSumUtilization} + ${core.utilization} =`);
		cpuSumUtilization += core.utilization;
		process.stdout.write(` ${cpuSumUtilization}\n`);
	});
	process.stdout.write(`${cpuSumUtilization} / ${hostCore.length} = `);
	averageCPUUtilization = cpuSumUtilization / hostCore.length;
	process.stdout.write(`${averageCPUUtilization}\n`);
